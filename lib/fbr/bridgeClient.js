 ...
 * @param {{ submissionId: string, invoice: object, idempotencyKey?: string, fbrToken?: string, environment?: 'sandbox'|'production' }} params
 * @returns {Promise<{ ok: boolean, status: number, envelope: any }>} 
 */
async function callBridge(action, { submissionId, invoice, idempotencyKey, fbrToken, environment = 'sandbox' }) {
