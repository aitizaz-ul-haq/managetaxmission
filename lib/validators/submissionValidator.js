...
const VALID_BUYER_TYPES = ['Registered', 'Unregistered', 'Unregistered Distributor', 'Retail Consumer'];
...
function normalizeIdentifier(value) {
  // FBR NTNs can be 7-char alphanumeric (e.g. "F443122"), CNICs are always 13 digits.
...
export function validateSubmission(data) {
  ...
  else if (scenario === 'SN018' && submissionType !== 'services_fed_st_mode') {
    errors.push('Submission type must be services_fed_st_mode for SN018');
  } else if (scenario === 'SN019' && submissionType !== 'sale_tax_fed') {
    errors.push('Submission type must be sale_tax_fed for SN019');
  }
  ...
  if (!invoiceDate) errors.push('Invoice date is required');
    ...
    const currentLocalDate = getStartOfToday();
    const selectedDate = new Date(`${invoiceDate}T00:00:00`);
    if (selectedDate > currentLocalDate) errors.push('Invoice date cannot be in the future. Please select the current or past date.');
  }

  const taxPeriodYear = Number(data.taxPeriodYear ?? firstItem.taxPeriodYear ?? new Date().getFullYear());
  const taxPeriodMonth = Number(data.taxPeriodMonth ?? firstItem.taxPeriodMonth ?? new Date().getMonth() + 1);
  const taxPeriodDate = getFirstDayOfMonth(taxPeriodYear, taxPeriodMonth);
  const currentDate = getStartOfToday();
  const earliestAllowedPeriod = new Date(currentDate.getFullYear(), currentDate.getMonth() - 5, 1);
  if (!taxPeriodYear || !taxPeriodMonth) {
    errors.push('Tax period is required');
  } else {
    if (taxPeriodDate > currentDate) {
      errors.push('Tax period cannot be in the future. Please select the current or past month.');
    }
    if (taxPeriodDate < earliestAllowedPeriod) {
      errors.push('Tax period must be within the last 6 months. Please select a current or recent month.');
    }
  }
  ...
  else if (!/^[0-9A-Z]{7}$|^[0-9]{13}$/.test(sellerNtn)) errors.push('Seller NTN/CNIC must be 7 characters (NTN) or 13 numeric digits (CNIC)');
  ...
  else if (!/^[0-9A-Z]{7}$|^[0-9]{13}$/.test(buyerNtn)) errors.push('Buyer NTN/CNIC must be 7 characters (NTN) or 13 numeric digits (CNIC)');

  const invoiceNumber = normalizeText(data.invoiceNumber) || normalizeText(firstItem.invoiceNumber);
  if (!invoiceNumber) errors.push('Please enter the internal document number');

  const buyerType = normalizeText(data.buyerType) || normalizeText(firstItem.buyerType);
  if (!buyerType) errors.push('Please select a buyer type');
  else if (!VALID_BUYER_TYPES.includes(buyerType)) errors.push('Buyer type must be one of the valid buyer type options');
  ...
  if (!itemList.length) {
    ...
    const expectedSaleType = scenario === 'SN018'
    ...
    itemList.forEach((item, i) => {
      ...
      const rowBuyerType = normalizeText(item.buyerType);
      if (!rowBuyerType) errors.push(`Item ${n}: Please select a buyer type`);
      else if (!VALID_BUYER_TYPES.includes(rowBuyerType)) errors.push(`Item ${n}: Buyer type must be one of the valid buyer type options`);
      if (!normalizeText(item.invoiceNumber)) errors.push(`Item ${n}: Please enter the internal document number`);
      if (!item.saleType?.trim()) errors.push(`Item ${n}: sale type is required`);
      else if (expectedSaleType && normalizeText(item.saleType) !== expectedSaleType) {
        errors.push(`Item ${n}: sale type must be ${expectedSaleType}`);
      }
